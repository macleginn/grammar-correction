...
def preprocess_alignment(alignment_dict):
    en_degrees = Counter()
    fr_degrees = Counter()
    unaligned_en = []
    unaligned_fr = []
    ...
    for key, val in alignment_dict.items():
        if key == 'X':
            unaligned_fr.extend(val)
        elif val == ['X']:
            unaligned_en.append(key)
        else:
            for tail in val:
                en_degrees[key] += 1
                fr_degrees[tail] += 1
                real_edges.append((key, tail))
...
def get_edge_label_report(corpus, path_str, cursor):
    ...
    result = []
    temp_dict = {}
    for i, record in enumerate(cursor.execute(f'select `en`,`ru`,`alignment`,`verified` from `{corpus}`')):
        en, ru, alignment, verified = record
        if verified != 1:
            continue
        en_n, en_g = conll2graph(en)
        ru_n, ru_g = conll2graph(ru)
        pprint(alignment)
        alignment_dict = json.loads(alignment)
        (
            unaligned_en,
            _,
            one_to_many_en,
            one_to_many_ru,
            resulting_edges
        ) = preprocess_alignment(alignment_dict)
    ...
    for item in sorted(
        ...
        if len(item[1]) < 10:
            break
        result.append([f'{item[0]}: {len(item[1])}', item[1]])
...
if __name__ == "__main__":
    ...
    cursor = conn.cursor()
    result = get_edge_label_report(argv[1], argv[2], cursor)
    import json
    with open('path_mappings_test.json', 'w') as out:
